...
As global digital advertising budgets continue to grow, overtaking TV as the dominant form of advertising [GET REFERENCE FROM ARI - https://www.emarketer.com/Article/US-Digital-Ad-Spending-Surpass-TV-this-Year/1014469], the problem of measuring the \textit{effectiveness} of ad campaigns is ever-important: 
...
Sections \ref{sec-repeat} and \ref{sec-contam} describe how our methodology needs to be modified to handle the complications of recurring identifers and ID contamination, respectively. 
In Section \ref{sec-results} we present experimental results of actual Ad Impact measurements (and confidence intervals) for 9 campaigns, and show a detailed numerical example of our methodology applied to one campaign.
Section \ref{sec-related} concludes with a discussion of related work.  


%\todo{Set up the context, and outline plan of paper.}
%\todo{exposed instead of treated}
...
We will present a sequence of increasingly precise formulations of this question, setting the stage for a rigorous statistical framework. 
...
$$
which is the difference between the average response rates of treated and untreated consumers, both of which are observable. In general $S$ would not be equal to the ATT: The ATT measures {\em causation} whereas S merely measures {\em association} \cite{elwert2013graphical}. To see why $S$ may not always estimate $ATT$ correctly, consider the following decomposition of $S$:
...
We saw above that when the selection bias is zero, then the standard estimator $S$ is a good estimate of $ATT$. Absence of selection bias is a special case of a more general condition under which we can estimate $ATT$ (which involves a counterfactual) from observable expectations:
...
in general the actual outcome \textit{will} depend on exposure-status. Ignorability of exposure is a statement about potential outcomes: 
...
We provide here a simplified view of the mechanisms that the describe the operation of a DSP, as pertains to buying and delivering digital advertising. We will specifically describe the buying mechanism of Real Time Bidding (RTB), but the general principles laid out here apply to any ad-devliery contexts where similar event-level logging (as described below)and data flows exists, and the resulting methodology can be readily applied to those contexts as well. 
%(see Figure \ref{fig-dsp}). - NOTE I KILLED THE FIGURE
...
The bid request can be viewed as a tuple containing several fields, and in our present context the only ones that matters are $(u, r, e)$ where:
\begin{itemize}
	\item $u$ is the {\bf userID}. For example, this could be a cookie ID from a web browser, or a mobile advertising ID from a mobile app. 
...
%\includegraphics[width=0.7\textwidth, angle=270]{dsp.pdf}
%\caption{Operation of a DSP. The numbers on the arrows indicate the sequence of events, starting from a user visiting a %publisher site (or mobile app), and culminating in an ad being displayed.}
%\label{fig-dsp}
%\end{figure}

%\todo{Fix the DSP diagram}
%ARI KILLED IT, BUT IF YOU WANT TO INCLUDE, LET'S FIX IT TOGETHER

A DSP typically has hundreds or thousands of clients (advertisers) on whose behalf it submits bids throughout the day. 
However in response to a given bid request, only a subset advertisers may be eligible to bid due to several factors such as: (a) advertisers' targeting requirements, i.e., the types of consumers they want to reach, the contexts in which they want to reach them, etc. (b) governing campaign criteria such as the available budget and desired frequency of exposure, or (c) publisher restrictions on which types of advertisers they will accept. 
...
\iffalse
Since the DSP typically can only submit one bid to the exchange (though there are exceptions, which we ignore here because it is orthogonal to the methodologies we describe), it will need to determine: (a) which of the eligible clients (if any) gets to submit a bid, and (b) how much it should bid. This process typically involves determining each eligible advertiser's "optimal" bid price, and picking the one with the greatest optimal bid price.
Determination of the optimal bid price is often the result of a combination of ML-based predictions of the probability that viewing the ad leads to the desired consumer response (site visit, purchase, etc.), called the predicted conversion rate (CVR). We ignore details of the bid-determination process; 
\fi
For our present purpose the only relevant aspect is that it is either empty (for example if no ad campaigns are eligible) or results in a {\bf bid submission} tuple which we can think of as containing the bid-request tuple $(u,r,e,c,b)$ which contains the entries $u,r,e$ from the bid request, plus two additional entries $c,b$ where:
\begin{itemize}
...
Finally, if the DSP won the auction, it will serve an ad from campaign $c$ to the consumer $u$ using the session information associated with the bid request $r$, and log a record of the win. If it lost the auction, there is no action taken, other than possibl logging a record of the loss. 


\subsection{Logging}

In order to be able to optimize the operation of all aspects of a DSP (including determination of optimal bid prices, implementing targeting, etc.), it is essential that the system conducts {\em logging} of not only of bidding events but also of consumer behaviors, via suitably placed tracking mechanisms (also known as "beacons" or "pixels") on advertiser website, mobile apps, and other digital properties. Machine learning algorithms can make use of such logs to improve the efficacy of the DSP algorithms over time. It turns out that logging is also crucial for measuring the causal effect of ads. 
...
It is important to realize that the Test/Control assignment is assuemd for now to be a function of the userID only, i.e., a userID is either a Test or a Control userID. So if the same userID appears multiple times in different bid requests, it will either always be in the Test group or always be in the Control group. This of course does not necessarily describe the state of affairs in reality, but we shall introduce an approach to address this complication in Section \ref{sec-contam}.
...
While the post-bid randomization idea is simple to implement and leads to a statistically sound estimation of the causal effect, it has one big drawback: since the Test/Control assignment is done {\em after} placing a bid, the advertiser must pay for the won impression, regardless of whether the consumer is served an actual campaign ad, or a PSA ad. Thus if the holdout fraction is 10\%, the advertiser essentially wastes 10\% of their ad budget on PSA ads, which in turn hurts campaign ROI by a correspoding amount. Moreover, given the low absolute value of typical response rates, larger holdout fractions may sometimes be required to improve measurement significance. Considering that annual digital advertising budgets for large advertiser can easily run into the 10s or even 100s of millions of dollars, this is a significant problem. Motivated by this consideration, we have designed a {\em pre-bid randomization} scheme, and a methodology to estimate $ATT$ under that scheme, which does away with the need to spend money on bids won against the control group. We describe this in detail in the following sections.
...
\begin{itemize}
	\item Phantom control impression $Imp(t, u, c_0, C)$ indicates that at time $t$, a bid was about to be submitted for userID $u$, but was not submitted because $u$ is a Control userID, i.e. $H(u,p) = C$.
...
\begin{definition}\label{def-ice-noncomp}
	textbf{Individual Causal Effect ($ICE$) Under Non-Compliance:} The potential outcome of a userID $u_i$ if it were assigned to Test, minus the potential outcome if it were assigned to Control:
\begin{equation}
...
%\todo{Talk about the other case $U_i=1$}
...
%\todo{3rd exprn needs to use ignorability; fix it as a lemma/property}
\begin{align}
ATT & =  E_i[ ICE_i | U_i = 1]   \nonumber \\
...
\begin{proposition} \label{prop-rtl-rcl}
...
\begin{lemma} \label{lem-att-prebid}
...
%We have been assuming there is only one campaign $c$ under consideration, but the methodology is easily extended to simultaneously measure the causal effect of multiple campaigns: each impression and event log record has a campaign ID, so we partition the logs by campaign ID and apply the above methodology to each partition independently. To avoid interference among multiple campaigns of the same advertiser, it is important to keep the test/control assignment function $H(u,p)$ the same across all tests; this will ensure that a userID's test/control assignment is the same across all campaigns.
%\todo{Ari wanted to add this -- perhaps rephrase}
%ARI: yes - in Section 1, and in the conclusion, I want to foreshadow "paper #2". But I removed it here.
...
I EDITED THIS SECTION HEAVILY IN TERMS OF THE ACTUAL PRINCIPLES. PLEASE REVIEW!

For this discussion, suppose advertiser $A$ is measuring the effectiveness of campaign $c_0$ on the DSP $D$. For brevity, we will say a campaign $c_1$ is \textit{related} to campaign $c_0$, if $c_1$ has an influence on a consumer's likelihood of performing campaign $c_0$'s desired outcome. In particular, campaign $c_0$ is of course related to itself. 

In general, it is possible that consumers in both the Test and Control groups of campaign $c_0$ are exposed to ads from related campaigns by Advertiser $A$, either from the DSP $D$ in question, from another DSP executing ads for $A$ in digital channels, or from advertising by $A$ in non-digital channels (e.g., traditional TV, print, radio, etc.). If exposure from such related campaigns is effectively random, in the sense that userIDs in Test and Control for campaign $c_0$ are equally likely to be exposed to related campaigns, and moreover that winner types and loser types in both groups are equally likely to be exposed to related campaigns, then the methodology presented here for measuring $ATL$ and $INC$ remains intact, but with two qualifications:
...
%	\item The same advertiser $A$ running a \textit{related} campaign (same or different from $c_0$) on a \textit{different DSP} could win this bid request. 
%\end{itemize}

%A subtle point to note in our Pre-Bid Randomization Procedure \ref{alg:pre-bid} is what happens when the DSP $D$ receives a bid request where the userID is in the Control group, and the bidder determines that the campaign under consideration $c_0$ is the best match: the DSP simply does not submit a bid. In this case it is entirely possible that this userID is exposed to another advertisement from a \textit{related} campaign $c_1$ in two different ways:
%\begin{itemize}
%	\item Campaign $c_1$ of advertiser $A$ could submit a bid (and win), on the same DSP $D$, or 
%	\item The same advertiser $A$ running a \textit{related} campaign (same or different from $c_0$) on a \textit{different DSP} could win this bid request. 
%\end{itemize}
%In both these cases, the winner-type userIDs in the Control group are not really "control" at all: they may be exposed to an ad that could be better or worse in eliciting the desired response from consumers. Thus the counterfactual response rate $R'_{CW} = E[Y(0) | U=1]$ would not represent what we wish it to represent, namely the response rate of the winner-type userIDs \textit{with no exposure to any ad driving the same response as $c_0$}. 
%In effect, if Control users are exposed to another related campaign $c_1$, then we're comparing response due to $c_0$ versus the response due to (partial exposure to) $c_1$, whereas what we really want to compare is the response due to $c_0$ versus the response to "no ad". 
%In particular, the $ATT = R_{TW} - R'_{CW}$ could well turn out to be \textit{negative} because Control userIDs are exposed to a related campaign $c_1$ ad that is \textit{better} (i.e., generates more response) than the campaign $c_0$ ad whose effectiveness we are measuring. This is analogous to a situation in a randomized clinical trial of a drug, where the control group patients, rather than taking a placebo, actually take the same or a competing drug, which invalidates any drug effectiveness measurement. 

%We therefore make the following important assumption, which guarantees that the Control group is not exposed to any campaign related to the measured campaign $c_0$:

%\todo{Modify this based on Ari's comments}

To account for this, we recommend the following practices be followed when measuring the effectiveness of campaign $c_0$ of advertiser $A$ on DSP $D$:

%\begin{assumption}
%	\textbf{Exclusivity:} Suppose DSP $D$ is measuring the effectiveness of campaign $c_0$ of advertiser $A$. We assume that:
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item the other campaigns related to $c_0$ target a set of userIDs that is disjoint from the set of userIDs targeted by $c_0$. %This is often done for example when re-marketing campaign for a specific product "anti-targets" a prospecting campaign for the same product, i.e. they are designed to target mutually exclusive sets of users.
	\end{itemize}
\end{enumerate}
%\end{assumption}
...
We have developed a methodology to compute confidence bounds for both metrics (or any other similarly-defined metric) based on a simple Gibbs sampling scheme. Such schemes were proposed for example by \cite{Chickering1996} and \cite{Barajas2012}, but we believe our methodology, as well as its presentation, is simpler and more intuitive to understand. Moreover, unlike the estimation of \cite{Imbens1997a}, our methodology is relatively assumption-free, works for non-large samples, and does not rely on a normal approximation.
...
ARI ADDED THE BELOW, PLEASE CHECK IT.
Note that the introduction of these modifications means that different consumers in the Test group may be subject to different degrees of ad exposure (i.e., because the DSP may win only one bid opportunity against some Test-Winner userIDs, and multiple bid opportunities against other Test-Winner userIDs.). In theory, consumer response to advertising should be some function of the degree (i.e., frequency) of exposure. For the current purposes, the shape of this function irrelevant, in the sense that the response rates in question (i.e., $R_{T}$, $R_{TW}$, etc.) are simply aggregates over the corresponding populations. The degree of exposure that drove the response rates does not affect the calcuation of Causal Lift. What would be affected is the measured return on investment (ROI) of the campaign, which looks at how much spend was required to generated the observed lift, as higher frequency of exposure would entail higher cost and vice versa. ROI considerations will be visited in later work; the current approach seeks to quantify the Causal Lift independent of the spend that was required to produce it.

\iffalse
\begin{algorithm} 
 \caption{Conversion Credit Assignment for Recurring UserIDs}
    \label{alg-credit}
\begin{algorithmic}[1]
\Require 

$PV$: Post-view window of campaign being measured.
\For {each userID $u$} 
	\For {each bid opportunity $b$ of userID $u$}
		\State{Initialize credit to zero: $credit(b) \gets 0$}
	\EndFor
\EndFor	
\For {each userID $u$}
	\For {each conversion event $e$ of $u$}
		\State{Let $t$ be the time of event $e$;}
		\State{Let $W$ be the time-interval $[t-PV, t]$;} \Comment{ \textit{credit window}}
		\State{$B = $ time-ordered sequence of bid opportunities of $u$ in window $W$;}
		\If {$u$ is a Control userID}
			\State{Let $b$ be last bid opportunity in $B$;}
			\State{ $credit(b) =\gets credit(b) + 1$}
		\ElsIf {there is at least one WON bid opportunity in $B$;}
			\State{Let $b$ be the last WON bid opportunity in $B$;}
			\State{ $credit(b) \gets credit(b) + 1$}
		\ElsIf {there is at least one LOST bid opportunity in $B$;}
			\State {Let $b$ be the last LOST bid opportunity in $B$}
			\State{ $credit(b) \gets credit(b) + 1$}
		\EndIf
	\EndFor
\EndFor
...
Applying Procedure \ref{alg-credit} results in a (possibly zero) \textit{conversion credit} being assigned to every bid opportunity of each userID. Now for each userID (which is the unit of our analysis) we define the outcome as follows:

\begin{modification}
	For a given userID $u$, the \textit{outcome} $Y$ is the total conversion credit of all bid opportunities of $u$, as determined by Procedure \ref{alg-credit}.
\end{modification}
\fi
...
The methodology thus far has been based on the \textit{userID} as the unit of study, and essentially calculates the \textit{userID-level} Causal Lift (even when a userID occurs multiple times). However, as mentioned in the Introduction, userIDs are identifiers that in general are not unique to a given consumer and may change over time. In reality, though we have been using the term "userID," these identifers actually correspond to devices, not the actual users of those devices. The most well-know type of userID is a browser cookie, which identifies the individual instance of the web browser being used on a particular device. Increasingly, these cookies are either deleted periodically by users or not accepted/supported by browsers to begin with, such that they do not necessarily persist across different observations of the same consumer. Thereofre, a given consumer could appear with a distinct (cookie-based) userID, or in fact no userID, on every bid opportunity. We refer to this as cookie contamination. It is noted that while cookies are unique to a particular browser, they are not unique to a consumer nor to a device (as a given consumer might use multiple browsers on the same device - in fact, this behavior is common). Another common type of userID is a mobile advertising ID (such as an Apple IDFA or Google Android ID), which is used to identify the mobile device used by a consumer. However, consumers are increasingly using multiple connected devices (smartphone, tablets, etc.) with that trend only expected to continue. Thus, while mobile advertising IDs are generally persistent (i.e., stable over time) and are unique to a device, they are not unique to a consumer. That means bid opportunities to a given consumer that are delivered on different devices owned by that consumer, will correspond to different userIDs. We refer to this as cross-device contamination. The reverse scenario is also possible, whereby a given userID (either cookie or mobile advertising ID) may correspond to multiple consumers (as might be the case with shared household or family usage of a single device), though the single-identifier-to-many-consumers scenario is in practice relatively minor compared to the many-identifiers-to-single-consumer scenario. 

Both cookie contamination and cross-device contamination are highly prevalent, and are not mutually exlcusive with respect to a given consumer. The fact that the commonly-used userIDs are not unique to a consumer, and may not remain persistent over time, means there is a many-to-many relationship between userIDs and consumers. This means that, in general, some of the userIDs for a given consumer will end up in the Test group and some in the Control group, leading to contamination of the results. In particular, since the consumer in question will have been exposed to the ad campaign, \textit{all} of the userIDs associcated with that consumer would subsequently exhibit the response rates of an exposed userID, including the ones in Control. In fact, this is another form of SUTVA violation (as described in Section \ref{sec-repeat}): there is interference among units, and specifically, the exposure status of a userID influences the potential outcomes of another userID associated with the same actual consumer. Intuitively, one expects that ID contamination will lead to a \textit{dilution} in the measured lift, i.e., the measured userID-level lift would \textit{understate} the true consumer-level lift. The following subsection uses a simple model to quantitavely illustrate this effect.



\subsection{Impact of ID contamination}
Other authors have explored the quantitative impact of ID contamination in detail. For example, \cite{coey2016people} present an analysis of the impact of cookie contamination on Ad Lift measurement, using both simulations and analytical methods. For illustration purposes here, we consider the following simple "toy model" scenario (illustrated in Figure \ref{fig-contam-dilution}) describing cookie contamination and its directional effects on Ad Lift measurement:
\begin{enumerate}
	\item Each consumer has exactly $k$ cookie-based userIDs (or just "cookies" for short). A common corresponding real-world scenario would be a consumer who periodically clears the cookies in their browser, such over the time period in question for the lift analysis, they have $k$ distinct cookies. We can think of cookies as partitioned into clusters of $k$ cookies each, and each cluster corresponds to an actual consumer.
	\item Each cookie is randomly assigned to Control ($C$) with probability $p$, and to Test ($T$) with probability $1-p$.
	\item Win rate is 100\%, i.e., \textit{all} Test cookies are exposed to the ad campaign under study.
	\item There are two types of consumers: type-$C$, i.e., those with all $k$ cookies in Control (this happens with probability $p^k$), and type-$T$, i.e., those with at least one cookie in Test (which happens with probability $1-p^k$). Since every Test cookie is exposed to an ad, the \textit{cookie}-level randomized assignment to Test/Control results in a random assignment to type-$T$ and type-$C$ at the \textit{consumer} level, with the probability of assignment to type-$C$ being $p^k$.
	\item A type-$T$ consumer has a conversion probability of $r_t$, and \textit{each} of their $k$ cookies is \textit{equally likely}\footnote{Note that we are implicitly assuming that a consumer's response probability is flat regardless of how many times they are exposed to an ad campaign. In reality, we expect repsonse to be a diminishing fucntion of frequency of exposure, but we assume frequency independence here in order to arrive at a simple, quantitative, directional estimate of the impact of cookie contamination.} to convert, i.e., they each have a conversion probability of $r_t/k$. 
	\item A type-$C$ consumer has a conversion probability of $r_c$, and \textit{each} of their $k$ cookies is \textit{equally likely} to convert, i.e. they each have a conversion probability of $r_c/k$.
...
\caption{\small A simple cookie contamination scenario where each consumer has exactly three cookies, and each cookie (represented by a circle) is randomly assigned to Control with probability $p$ (and to Test with probability $1-p$). The cookie distributions of three consumers are shown. Each row of cookies is associated with a distinct consumer. The two Response Rate columns show the response rates of the consumer, and that of each cookie of the consumer.}
\label{fig-contam-dilution}
\end{figure}

I DON'T THINK THE GRAPH EXPLANATION IS CORRECT. ARE THOSE BID OPPS ONLY, OR OUTCOMES OR BOTH? LET'S DISCUSS.
ALSO, YOU ARE THINKING OF COOKIES HERE AS EFFECTIVELY CROSS-DEVICE. IF YOU WANT TO DESCRIBE COOKIES, YOU NEED TO ASSUME SOME PROBABILITY THAT THE COOKIE WILL PERSIST BETWEEN BID OPP AND DESIRED OUTCOME (A LA YONI) SINCE UNLIKE MOBILE IDS, THE KEY ASPECT OF THE COOKIE IS THAT IT IS NOT PERSISTENT. WE NEED TO DECIDE IF WE ARE TALING DIFFERENTLY ABOUT COOKIE AND CROSS DEVICE.
I SUGGEST ADDING IN q=PROBABILITY THAT COOKIE PERSISTS IN BETWEEN BID OPP AND OUTCOME AND GENERALIZING THE MATH. THEN WE CAN CLAIM CROSS-DEVICE IS THE CASE WHERE q=1 AND EXPLORE THE GENERALIZATION THAT THERE IS A DISTRIBUTION OF k.

We can then show the following:

\begin{proposition}\small [Dilution due to Cookie Contamination]
	In the above scenario where each consumer has $k$ cookies, and each cookie is assigned to Control with a probability $p$, all Test cookies are exposed to ads, and the true consumer-level causal Ad Lift (in the sense of $ATL$, Definition \ref{def-lift}) is $a$, then the lift measured at cookie level will be 
...
\end{proposition}
\begin{proof}\small
Let $R_t$, $R_c$ be the measured Test and Control response rates respectively, at the cookie-level.
All Test cookies must belong to a type $T$ consumer and hence have conversion probability $R_t = r_t/k$. 
On the other hand, the conversion probability of a Control cookie will depend on whether or not it is "contaminated", i.e., whether or not a "sibling" of that cookie (i.e., one that belongs to the same consumer) is in Test.
...
Thus the measured Control cookie response rate would be 
\begin{eqnarray}
...
The above result shows that as the number of cookies $k$ per consumer increases, the measured cookie-level lift drops \textit{exponentially} in $k$, since the term involving $p^{k-1}$ in the denominator can be neglected for large $k$ values, and the expression reduces to $a p^{k-1}/(1 + a)$. As a sanity check it is worth noting that the $ATL$ expression (\ref{eq-atl-contam}) reduces to $a$ when $k=1$. It is also noted that the above implicitly assumes that while consumers have $k$ cookies each, those cookies are stable (i.e., persist) for at least the time between the bid opportunity and the desired outcome. In fact, this is often not the case, which would result in a depression of both the Test and Control response rates (assuming that cookie non-acceptance and cookie deletion are equally common among Test and Control consumers), effectively decreasing the signal-to-noise ratio.

This result might lead one to conclude that at worst, contamination would drive the Causal Lift estimation to zero. 
This makes intuitive sense: when contamination is severe, every Control cookie essentially has the response rate of a Test (exposed) cookie, and \textit{as long as the composition of the type-$C$ and type-$T$ consumer populations is the same}, the measured causal Ad Lift would be at worst zero. Our research, using an unbiased truth set whereby the underlying (anonymized) identity of each consumer is actually known, shows that the {\em average} number of browser cookies per consumer (specific to a particular DSP and examined over a timeframe of INSERT TIMEFRAME) is typically in the (INSERT X-X NUMBER RANGE) range, with a long tail of consumers exhibiting {\em far} more cookies per consumer. While the model above assumed a fixed number of cookies per consumer rather than a distribution, the fact that consumers on average tend to have many cookies would lead one to expect significant reduction in lift, if not a zero result. 

Yet, the situation is even worse than that! In the above scenario, zero (or extremely low) measured lift resulted from high $k$ under the assumption that each consmer has exactly $k$ cookies. If we remove this assumption, and instead assume the population consists of some consumers with just one cookie, as well as others with multiple cookies, then it is possible for the composition of the Test and Control populations to have different responsiveness to ads, due to a combination of two phenomena (which we have observed in our data):
\begin{enumerate}
\item Consumers with multiple cookies constitute a higher fraction of the Control population than Test, and 
ISN'T THIS PURELY A MATHEMATICAL RESULT, BECAUSE IF YOU HAVE MULTIPLE COOKIE YOU ARE MUCH LESS LIKELY TO HAVE ALL OF THEM SHOW UP IN CONTROL? IS THIS WHAT YOU PROVE BELOW? IF SO, I'LL MENTION. IF NOT, THIS SHOULD BE BRIEFLY EXPLAINED BELOW
\item the baseline (no-exposure) response rate of multiple-cookie consumers is higher than that of single-cookie consumers.
\end{enumerate}
The second phenomenon is quite plausible when one imagines cookies as corresponding to different \textit{devices} a consumer might use (e.g. desktop, tablet, mobile phone, etc.), and consumers with multiple devices have a higher propensity to convert even in the absence of any advertising.

ARI: I DON'T UNDERSTAND THE ABOVE CLAIM. FIRST OF ALL, MULTIPLE COOKIES DON'T TYPICALLY CORRESPOND TO MULTIPLE DEVICES, BUT RATHER TO REPEATED INSTANCES OF THE SAME DEVICE. SECOND, WHY ARE MULT-DEVICE USERS MORE LIKELY TO CONVERT??

NEED TO DISCUSS THIS PART TOGETHER. I CAN THEN GENERALIZE TO DISCUSS COOKIES AND CROSS DEVICE SINCE I HAVEN'T FULLY EDITED THIS PART.
...
\begin{proposition}\small [Multi-Device Skew]
...
Clearly there is a tradeoff with this approach: for higher $k$ we will have less data, resulting in wider confidence bounds around the measured lift. In practice, we have found the best results for $k=2$ or $k=3$, which produce noticeably different results vs. $k=1$. This is consistent with empirical studies (e.g., \cite{coey2016people}) showing that a vast majority ($> 70\%$) of consumers have fewer than 4 cookies, and that digital consumers own, on average, about 3.5 connected devices [REFERENCE https://www.globalwebindex.net/blog/digital-consumers-own-3.64-connected-devices] though most of their media consumption (and hence ad exposure) occurs on 2 or 3 devices (e.g., smartphones, laptop, and tablets, as opposed to gaming consoles, wearables, etc.). Thus, using $k=2$ or $k=3$ helps prevent ID contamination from overwhelming lift measurements, avoiding singletons and gearing the analysis around the empirically expected value for $k$.
...
Issues: 
\begin{itemize}
\item Hold out by userID vs cid, 
\item cid-enhance
\item restrict to dr2
\item decontamination
\item Add the result about LB <= ATL (i.e rt-rc level lift is lower bound on ATL)
\end{itemize}


\section{Complication: Fractional attribution} \label{sec-frac}
\todo{Should we do this?}
\fi
THINK ABOUT WHETHER TO INCLUDE result about LB <= ATL (i.e rt-rc level lift is lower bound on ATL)

\section{Experimental Results} \label{sec-results}

In table \ref{tab-results} we show results of the Causal lift ($ATL$) computation using the modified methodology of Section \ref{sec-contam}, for nine campaigns using bid opportunity data over a 30-day period ending March 21, 2017. 
...
ARI: I WANT TO KILL ROWS 1 AND 9 HERE -----------------------------------------------
...
\hline \hline 
1   & 5   & 4   & 2   & 63    & -15 & 4   & 30   & 23   & 22   & 41   & 39 & 925,854  & 2,100   & 359,786 & 1,477  & 63,660  & 140   \\
2   & 63  & 39  & 25  & 99    & 12  & 62  & 167  & 63   & 49   & 65   & 56 & 263,501  & 1,670   & 148,058 & 955   & 16,065  & 79    \\
3   & 18  & 15  & 14  & 100   & 7   & 18  & 30   & 57   & 52   & 93   & 42 & 2,195,456 & 12,609  & 918,316 & 8,573  & 145,216 & 748   \\
4   & 17  & 14  & 8   & 91    & -3  & 16  & 44   & 46   & 43   & 53   & 47 & 734,135  & 3,390   & 346,656 & 1,840  & 69,511  & 296   \\
5   & 534 & 84  & 6   & 100   & 74  & 335 & 3597 & 3    & 2    & 8    & 13 & 4,938,065 & 1,423   & 657,002 & 503   & 459,553 & 93    \\
6   & 153 & 60  & 6   & 94    & -11 & 109 & 1317 & 4    & 3    & 9    & 15 & 2,409,520 & 902    & 364,234 & 343   & 110,991 & 32    \\
7   & 5   & 5   & 46  & 100   & 3   & 5   & 8    & 629  & 610  & 887  & 40 & 1,955,475 & 122,968 & 787,613 & 69,874 & 205,131 & 12,520 \\
8   & 12  & 11  & 1   & 65    & -23 & 9   & 72   & 2    & 2    & 6    & 24 & 2,833,414 & 511    & 681,506 & 380   & 198,932 & 33    \\
9  & 5   & 4   & 55  & 64    & -16 & 5   & 34   & 1325 & 1287 & 1272 & 69 & 1,531    & 203    & 1,060   & 135   & 1,530   & 197  
\end{tabular}
\end{sidewaystable}


To illustrate how our methodology is applied, let us consider the calculations for campaign 2 (second row in the table):
...
There is a strand of literature addressing the challenging problem of estimating the causal effect of advertising based on \textit{observational} data, i.e., from data collected during the normal course of campaign delivery, without setting up explicit randomized control tests. One such paper is \cite{stitelman2011estimating} which considers several ways to address the chief difficulty in estimating causality from observational data: the exposed and unexposed populations will not in general be equivalent. The authors consider various ways of correcting for this non-equivalence. Essentially these approaches rely on doing a counterfactual or "what-if" analysis \textit{analytically} via fitted models/estimators rather than experimentally via randomization. An important requirement in these approaches is that \textit{all} potentially confounding variables (i.e., those that affect assignment to the treatment group and also affect response rates) are observable, which is rather a strong requirement and one which we believe does hold in the real world. Thus, these approaches tend to be assumption-driven (in terms of the models/estimators used), prone to unknown biases in the construction of treatment populations, and have proven to be difficult to validate in practice, leaving many adveritsers questioning if the results can really be believed. While observational approaches have the appeal of zero  opportunity cost to advertisers (who would miss out showing ads to the Control group in a randomized test), we believe these downsides make such approaches sub-optimal for reliable Incrementality measurement. 

\iffalse
\todo{Say more here. We know in practice the results are inferior}

Still the observational approach has an appeal in the sense that there is no opportunity cost to advertisers (who would miss out showing ads to the control group in a randomized test). 
\fi
...
%Their analysis relies on the fact that in the environment they studied, Facebook had access to ground-truth about which cookies belong to which consumers -- this is not applicable to a DSP in general.
% THAT IS TRUE FOR A DSP - THAT IS CONNECTEDID!!!!!!!!!!!!!!!

% Facebook network effects paper: \cite{Eckles_Design_2014}
...
In future work, we intend to explore various generalization and extensions of this approach. These include: 1) adapting the methodology to decscribe the Ad Lift from multiple campaigns and/or digital adveritsing channels, inlucding interactions effects, 2) specifying how such Ad Lift measurements should correctly advertising budget allocation decisions, and 3) Unifying this approach with a methodology for the valuation of individual bid opportunities, in a manner that accounts for incrementality at the bid-opportunity level.

DO YOU WANT TO INCLUDE THIS SECTION OR IS IT TOO RISKY???????????????

\section{Acknowledgements}\label{sec-acknowledge}

DO WE WANT TO ACKOWLEDGE ANYONE? E.G., for "helpful discussions" and problem-solving sessions? such as Jaynth, Jason, or Yoni?


\bibliography{papers-mar29,all_papers,mendeley-bib}{}
%\bibliography{papers-mar15}
...
%\begin{example}
%\small
%Suppose an ad  has no causal effect, i.e. $Y_i(0) = Y_i(1)$ for every consumer $i$ (so $ATT = ACE = 0$) and the data we've collected is such that the average {\em un-exposed} response-rate among unexposed users is 5\%, while the average {\em un-exposed} (counterfactual) response-rate among exposed users is only 1\%, i.e. 
%	$$E[Y(0) \\| W = 0] = 5\%, \;\;\; E[ Y(0) \\| W = 1] = 1\%$$
%	In other words, there is a serious {\bf selection bias} between the exposed and un-exposed populations, i.e. exposed consumers have a much lower baseline (i.e. un-exposed) response-rate than un-exposed users.
%	Now since $ATT=0$, among exposed users the observed response-rate is the same as their {\em unexposed} (counterfactual) response-rate:
%$$
%E[ Y(1) | W = 1 ]  = E[ Y(0) | W = 1 ] = 1\%
%$$	
%	This leads to $S = 1\% - 5\% = -4\%$, which of course is not a good estimate of $ATT = 0\%$.	
%	\qedsymbol	
%\end{example}
%
%In the above example there was a statistical dependence between treatment (the ad exposure variable $W_i$) and the potential outcomes $Y_i(0), Y_i(1)$: of the 
%
