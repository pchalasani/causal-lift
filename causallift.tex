...
There are, however, approaches to infer causality or incrementality from such \textit{observational data} (see, e.g. \cite{Austin_An_2011} and other references in the Related Work Section \ref{sec-related}). We believe these approaches are best suited for unit/individual/ITE level (where randomization is not well defined) as opposed to population or aggregate level, where randomization is well defined) incrementality estimation, so we do not consider these approaches in the current work. Experimentation is always superior but ONLY when we are ABLE to do it or when it's meaningful. There's no notion of randomizing an individual but it can be done at population level (at least for aggregate-level causal estimation) they tend to be assumption-driven, prone to biases, and difficult to validate in practice. Instead we seek to derive unbiased estimates of causality using an experimental approach that builds on established frameworks, accounts for real-world factors, and can be easily reproduced and verified.
...
We refer to this as \textit{cookie contamination}. Note that while cookies are DETERMINISTIC AT A SINGLE POINT IN TIME uniquely identifies a particular browser at a particular point in time (as long as cookies are not deleted), they are not unique to a consumer nor to a device (as a given consumer might use multiple browsers on the same device -- in fact, this behavior is common).
...
%Ari  comment below -- insert numbers
...
ENSURE CONSISTENT WITH ABOVE POV ON OBSERVATIONAL APPROACHES!!!!!!!!
