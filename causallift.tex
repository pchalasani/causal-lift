...
MediaMath (a DSP) has developed the first practical, production-deployed, statistically sound randomization-based methodology for true causal ad effectiveness (or Ad Lift) measurement by a DSP (or similar buy-side digital advertising execution system that may not have full control of underlying RTB auction mechanisms).
...
that stem from the fact that digital advertising is delivered to consumers  tracked by identifiers (e.g., cookies, mobile advertising IDs) which may not be stable over time. One such complication is the repeated occurrence of identifiers, leading to interference among observations. 
Another is due to several distinct identifiers being associated with the same human user, leading to "contamination" with some of their identifiers being assigned to the Treatment group and others to the Control group. 
...
provides a rigorous, self-contained, highly-readable, readily-implementable treatment of all relevant concepts.
\end{abstract}
